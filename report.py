...
def clockify_time_to_object(time_string):
    ...
    return time.strptime(time_string.replace("PT", "").replace("M", "").replace("H", ":"), time_format)
...
entries = client.time_entries.get_time_entries(workspace_id, current_user['id'], {'start': workweek[0], 'end': workweek[1]})
